...
namespace DungeonCrawler
{
    internal class MainGame
    {
        ...
        public void RunGame()
        {
            ...
            GenerateNewGame(width, height, this);
        ...
        private void GenerateNewGame(int width, int height, MainGame mainGame)
        {
            DungeonGenerator generator = new();

            List<DungeonTile> map = generator.GenerateDungeon(width, height, this);

            SpawnPlayer(FindPlayerSpawn(), map);

            SpawnEnemy(FindEnemySpawn(10, map), map);

        }

        private void SpawnEnemy(List<Coordinate> coordinates, List<DungeonTile> map)
        {

        }

        private List<Coordinate> FindEnemySpawn(int NumberOfEnemies, List<DungeonTile> tileList)
        {
            List<Coordinate> coords = new List<Coordinate>();
            for (int i = 0; i < NumberOfEnemies; i++)
            {
                coords.Add(ConsoleHelper.GetRandomTile(width, height, tileList, false));
            }
            return coords;
        }
        ...
        private Coordinate FindPlayerSpawn()
        {
            return new Coordinate(rnd.Next(2, width - 2), rnd.Next(2, height - 2));
        }

        private void SpawnPlayer(Coordinate spawnCoords, List<DungeonTile> tileList)
        {
            DungeonTile playerSpawnTile = tileList.Where(item => item.coords.x == spawnCoords.x && item.coords.y == spawnCoords.y).First();
            Character playerCharacter = new Character().InstantiateCharacter(playerSpawnTile);
            playerSpawnTile.SetOccupant(playerCharacter);
        }
